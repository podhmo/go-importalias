...
- **v1.7からの変更点**: `strict`の意味を「多数決自体を無効化して報告を保留する」から「設定ファイル生成時に複数alias候補を配列として保持する」に変更した（11.23節）。
- **現時点で未確定の論点**: 11.22節（識別子衝突チェックのより精密な判定）は方針のみ確定しており、実装は今後のイテレーションに持ち越されている。加えて、vet analyzer側の`strict`フラグは設定ファイルを生成しないため実質的な意味が薄い（`docs/01decision.pre.md` PRE-24）。
...
- **DEC-2.2**: 設定ファイルの自動生成・更新（FR-7.3）では、diffを安定させるため以下を規則とする。
  ...
  - 単一候補としての無alias（`Resolved == ""`）は生成・更新時に出力しない。明示的に設定ファイルへ書かれた`""`は「aliasなしが正」を意味するが、生成物では通常の無aliasを省略してconfigを小さく保つ。無aliasが複数候補の一つである場合は、候補配列（例: `["", "foo"]`）として残す。
...
- **DEC-3.2**: `strict`（FR-4.7）は設定ファイルには保存せず、実行時オプションとしてのみ提供する。
  - CLIモード: `-strict`（bool, デフォルト `false`）。設定ファイル生成時に複数alias候補を配列として保持する。
  - vetツールモード: analyzerフラグ `-importalias.strict`（bool, デフォルト `false`）は互換性のため受け付けるが、vetモードは設定ファイルを生成しないため診断は従来通り多数決ベースとする。
...
| `-strict` | bool | `false` | 設定ファイル生成時に複数alias候補を配列として保持する。`-fix`とは併用不可 |
...
### 11.23 `strict`の新しい意味と報告・fixの分離

- **DEC-11.23**: `strict`は「多数決による診断対象の選定を止める」オプションではなく、「設定ファイル生成時に、同一パッケージ・同一import pathで観測された複数alias候補を多数決で1つに畳まず、配列として残す」オプションとする。
  - CLIのscan/config生成では、最多候補が1つに決まる場合でも、`-strict`指定時は設定ファイルには全候補を配列で書く。診断は従来通り、多数決で負けた側を報告する。
  - 最多候補が複数存在する通常のタイは、従来通り未解決タイとして報告し、設定ファイルにも候補配列を書く。
  - `-fix`と`-strict`は併用不可とし、同時指定時はCLIエラーにする。
  - vet analyzerは設定ファイルを書かないため、`strict`指定時も診断は多数決ベースのままとする。
  - 理由: `strict`の主用途を、人間があとで明示的に選べるように候補情報を欠落なくconfigへ残すことに置く。linterとしては、明らかな少数派利用箇所を引き続き報告した方が実用的であり、fixは候補配列が人間により1つへ解決されるまで安全に実行できないため分離する。
