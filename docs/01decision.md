...
- **v1.7からの変更点**: リポジトリの機能充足レビューで見つかった論点（第6回）をユーザーと確認し、以下を確定した（詳細は11節）。
  - `_test.go` を解析対象に含めるかは実行時フラグで制御し、デフォルトは各エントリポイントのホストツール（go vet / go/packages）の既定挙動に揃える（11.23節）。
...
- **DEC-11.23**: `*_test.go` ファイルを解析対象に含めるかは、設定ファイルではなく実行時フラグで制御し（DEC-2.4と同じくスキーマは拡張しない）、デフォルトは各エントリポイントのホストツールの既定挙動に揃える。
  ...
  - D2（CLI）: `-include-tests` フラグ（bool, デフォルト `false`）。ロードに使う `go/packages` の `Config.Tests` のデフォルト（`false`＝テストファイルを読まない）に準拠する。`true` 指定時は `Tests: true` でロードし、内部テストファイルはテスト拡張変種経由でパッケージの多数決に、外部テストパッケージは独立パッケージとして、それぞれ解析対象に含める。
  ...
  - 理由: ユーザー確認の結果、「フラグで `_test.go` の有効/無効を切り替え可能にし、含めるか除外するかのデフォルトは基盤ツールのデフォルト挙動の意思決定に準拠する」方針を採用した。これにより FR-7.9（`_test.go` を含む通常のGoソースファイルを対象とする）は、D1ではデフォルト挙動として、D2では `-include-tests` 指定時に充足される。
