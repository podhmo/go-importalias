...
- **v1.7からの変更点**: リポジトリの機能充足レビューで見つかった論点（第6回）をユーザーと確認し、以下を確定した（詳細は11節）。
  - `_test.go` を解析対象に含めるかは実行時フラグで制御し、デフォルトは `go vet` と同じく両エントリポイントで「含める」に揃える（11.23節）。
...
### 11.23 `_test.go` の解析対象への扱い（実行時フラグ）

- **DEC-11.23**: `*_test.go` ファイルを解析対象に含めるかは、設定ファイルではなく実行時フラグで制御し（DEC-2.4と同じくスキーマは拡張しない）、デフォルトは `go vet` の既定挙動に揃えて両エントリポイントで「含める」とする。
  - D1（go vet analyzer）: analyzerフラグ `-importalias.include_tests`（bool, デフォルト `true`。`go vet` の `-<analyzer>.<flag>` 慣習に従う）。`go vet` はテストファイルをデフォルトで解析対象にするため、これに準拠する。`false` 指定時は `*_test.go` の import は `internal/scan` の Occurrence にならず、外部テストパッケージ（`<pkg>_test`）も実質 no-op となる。
  - D2（CLI）: `-include-tests` フラグ（bool, デフォルト `true`）。`go vet` 挙動に揃え、デフォルトで `Tests: true` でロードし、内部テストファイルはテスト拡張変種経由でパッケージの多数決に、外部テストパッケージは独立パッケージとして、それぞれ解析対象に含める。`false` 指定時は `go/packages` の既定（`Tests: false`＝テストファイルを読まない）に従って除外する。
  - `Tests: true` で `packages.Load` が返すパッケージ集合の解釈は次のとおり: 合成テストバイナリ（`<pkg>.test`、package main、ビルドキャッシュ内のファイル）はスキャン対象から除外する。同一 PkgPath でプレーン版とテスト拡張変種の両方が返る場合は、ファイル集合が上位集合である拡張変種側（`Package.ForTest` が非空のもの）を採用する。
  - 理由: ユーザー確認の結果、「フラグで `_test.go` の有効/無効を切り替え可能にし、デフォルトは analyzer（go vet）側の挙動に合わせて両エントリポイントで含める」方針を採用した。本ツールは go vet を意識した立ち位置であり、`go/packages` の `Tests` デフォルトはロード機構上の既定であって、ツールとしての既定挙動は analyzer に揃える、という整理である。これにより FR-7.9（`_test.go` を含む通常のGoソースファイルを対象とする）は D1/D2 双方でデフォルト挙動として充足される。
  - 併せて、`go vet` が `-importalias.<flag>` 形式の analyzer フラグを `*.cfg` ファイルより前のコマンドライン引数として vettool に渡す挙動に合わせ、vetツールモードの起動判定（`looksLikeVetToolInvocation`）を「末尾が `*.cfg` で、それより前の引数がすべてフラグ形式」の場合に拡張した。
