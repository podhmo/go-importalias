...
# 第14回：strictオプションの意味変更

- **日付**: 2026-07-22
- **本書の位置づけ**: `strict`を、従来の「複数aliasがあれば多数決診断もタイ扱いにする」挙動から、「設定ファイル生成時に複数alias候補を配列として残す」挙動へ変更した回。

## やったこと

1. `internal/decide`は`Strict`指定時も最多候補が1つなら`WantAlias`と`Inconsistent`を計算し、同時に`TieCandidate`へ全候補を保持するようにした。
2. CLIの報告は、strict由来の候補配列があっても多数決で負けた側を報告し、通常の同数タイだけは未解決タイとして報告するようにした。
3. `-fix -strict`は、候補配列を残すモードと自動修正の責務が衝突するためエラーにした。
4. vet analyzerは設定ファイルを書かないため、`strict`指定時も診断は多数決ベースにした。

## 気づいたこと

1. `strict`の新しい主目的はconfig生成であり、vet analyzer側の同名フラグは実質的な意味が薄い。後方互換性のため残す判断にしたが、この扱いはPRE-24として未確定事項に残した。
2. 既存の`Decision.Tie`は「configには候補配列を書く」と「診断は未解決タイとして扱う」の両方に使われていた。今回、`Tie`かつ`Inconsistent`あり、という状態を許容することで変更範囲を抑えたが、将来的にはconfig生成用の状態と診断用の状態を型として分ける方が分かりやすい可能性がある。
