...
- [ ] **FR-4.7**: `strict` オプションが有効な場合、設定ファイル生成では同一パッケージ・同一import pathに複数のalias候補が存在する時点で、多数決で1つに畳まず、観測された全alias候補を配列として残す。検出・報告は、最多候補が1つに決まる場合は従来通り多数決で負けた側を対象にしてよい。

### 4.3 タイ発生時の挙動

- [ ] **FR-4.8**: タイが発生した対象については、自動修正を行わない。
- [ ] **FR-4.9**: タイ、または`strict`によって候補保持が必要になった対象については、設定ファイルに候補となる2つ以上のaliasを書き込む。
- [ ] **FR-4.10**: 設定ファイルに2つ以上の候補が並記された状態にある間は、当該対象への自動修正を行わない（人間が1つに絞るまで保留）。
...
- [ ] **FR-7.22**: `-strict` は設定ファイル生成のためのオプションであり、`-fix` と同時には指定できない。
...
| strictオプション | 設定ファイル生成時に複数alias候補を多数決で畳まず配列として残すモード |
